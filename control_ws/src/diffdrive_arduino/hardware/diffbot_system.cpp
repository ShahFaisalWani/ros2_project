  ...
  const rclcpp::Time & /*time*/, const rclcpp::Duration & period)
{
  ...
  // RCLCPP_INFO(
  //   rclcpp::get_logger("DiffDriveArduinoHardware"),
  //   "Encoder Counts - Left Wheel: %d, Right Wheel: %d",
...
//   // RCLCPP_INFO(
//   //   rclcpp::get_logger("DiffDriveArduinoHardware"),
//   //   "Left Wheel - cmd: %.4f, rads_per_count: %.4f, counts_per_loop: %d loop_rate: %f",
//   //   wheel_l_.cmd, wheel_l_.rads_per_count, motor_l_counts_per_loop, cfg_.loop_rate
//   // );

//   // RCLCPP_INFO(
//   //   rclcpp::get_logger("DiffDriveArduinoHardware"),
//   //   "Right Wheel - cmd: %.4f, rads_per_count: %.4f, counts_per_loop: %d loop_rate: %f",
//   //   wheel_r_.cmd, wheel_r_.rads_per_count, motor_r_counts_per_loop, cfg_.loop_rate
//   // );
  ...
  const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
  ...
  // Calculate the time since the last write
  auto now = std::chrono::steady_clock::now();
  auto elapsed_time = std::chrono::duration_cast<std::chrono::milliseconds>(now - last_write_time_);

  // Calculate the required delay based on loop rate
  int required_delay_ms = static_cast<int>(1000 / cfg_.loop_rate);

  // Check if the delay has passed
  if (elapsed_time.count() < required_delay_ms)
  {
  ...
  // Adjust the motor counts per loop as per the requirement
  if (motor_l_counts_per_loop > 0)
  {
    motor_l_counts_per_loop += 70;
  }
  else if (motor_l_counts_per_loop < 0)
  {
    motor_l_counts_per_loop -= 70;
  }

  if (motor_r_counts_per_loop > 0)
  {
    motor_r_counts_per_loop += 70;
  }
  else if (motor_r_counts_per_loop < 0)
  {
    motor_r_counts_per_loop -= 70;
  }
